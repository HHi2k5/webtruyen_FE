...
export default function AdminUsers() {
  ...
  const [res, setRes] = useState({ items: [], total: 0, page: 1, pageSize: 10 });
  ...
  useEffect(() => {
    api.listUsers({ q, page, pageSize: 12 }).then(setRes).catch(console.error);
  }, [q, page, dummy]);
  ...
  return (
    <div className="admin-container">
      <div className="admin-header">
        <div>
          <h2>{t('manageUsers')}</h2>
          <p className="sub-text">{t('adminSubTextUsers')}</p>
        </div>
      ...
      <div className="admin-toolbar">
        <div className="search-box">
          ...
          <input placeholder={t('searchByName')} value={q} onChange={e=>setQ(e.target.value)} />
        </div>
      </div>
      ...
      <div className="table-card">
        <table className="table admin-table">
          ...
          <tbody>
            {res.items.length === 0 ? (
              <tr><td colSpan="4" className="text-center empty-state">{t('noUsersFound')}</td></tr>
            ) : res.items.map(u=>(
              <tr key={u.id}>
                <td>
                ...
                <td>
                  <span className={`badge badge-${u.role === 'admin' ? 'admin' : 'user'}`}>
                    {u.role === 'admin' ? t('adminRole') : t('memberRole')}
                  </span>
                </td>
                <td className="actions text-right">
                  <button className="icon-btn btn-ghost" title={t('rename')} onClick={()=>renameUser(u.id)}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                  </button>
                  <button className="icon-btn btn-ghost" title={t('changeRole')} onClick={()=>updateRole(u.id)}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                  </button>
                  <button className="icon-btn btn-danger-ghost" title={t('delete')} onClick={()=>del(u.id)}>
